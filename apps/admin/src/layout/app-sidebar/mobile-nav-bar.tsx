...
function MobileNavBarButton({ to, activeOnSubpath = false, children, ...props }: MobileNavBarButtonProps) {
    ...
    return (
        <Button
            asChild
            className={`w-full min-w-9 max-w-16 rounded-full hover:bg-gray-200 ${isActive ? 'bg-gray-200' : 'bg-transparent'}`} {...props}
            variant="ghost"
            size="icon"
...
export function MobileNavBar() {
    ...
    return (
        <div className="safe-area-inset-bottom border-sidebar-border bg-sidebar/80 sidebar:hidden fixed bottom-0 left-0 right-0 z-50 h-[var(--mobile-navbar-height)] border-t backdrop-blur-md">
