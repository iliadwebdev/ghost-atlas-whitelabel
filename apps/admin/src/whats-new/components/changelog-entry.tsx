...
function ChangelogEntry({ entry }: ChangelogEntryProps) {
    return (
        <a
            className="hover:bg-muted/80 -mx-2 flex items-start gap-6 rounded-md p-2 transition-colors"
            data-test-entry
            href={entry.url}
        ...
        >
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-2">
                <h2 className="text-foreground mt-1.5 text-[17px] font-semibold" data-test-entry-title>
                    {entry.title}
                </h2>
